...
1. Runs BWA-MEM2 to align the single-end FASTQ files to a reference genome. (path to database hardcoded in script)
...
def run_bwa_mem2(sample, fastq_1, output_dir):
    """
...
def run_samtools(sample, output_dir):
    ...
    # Create file for output
    output_file = f"{output_dir}/bwamem2/{sample}_out_unconc.fastq"
    # Open output file
    with open(output_file, "w") as fastq_out:
        # Run the samtools fastq command and write output to file
        subprocess.run(command_fastq, stdout=fastq_out)
    # Print status message
...
def check_fastq(sample, output_dir):
    ...
    """
    # Path to fastq file
    fastq_file = f"{output_dir}/bwamem2/{sample}_out_unconc.fastq"
...
def main():
    ...
    with open(samplesheet, "r") as ssheet:
        ...
        for row in ssheet_reader:
            ...
            run_bwa_mem2(sample, fastq_1, output_dir)
            # Run samtools for the current sample
            run_samtools(sample, output_dir)
            # Check if fastq file contains reads
